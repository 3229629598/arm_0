...
RAM_PERSIST static uint8_t rc_rx_lost;

#if USE_RC_UART == 1

static uint8_t rc_data_buffer[2*RC_FRAME_LENGTH];
...
uint8_t parse_rc_data(uint8_t* pData){
	rc_ctrl_data.rc.ch0 = (int16_t)((int16_t)pData[0] | ((int16_t)pData[1] << 8)) & 0x07FF;
	if(rc_ctrl_data.rc.ch0 > RC_CH_VALUE_MAX || rc_ctrl_data.rc.ch0 < RC_CH_VALUE_MIN) return RC_CH_ERROR;
	rc_ctrl_data.rc.ch0 -= RC_CH_VALUE_OFFSET;

    rc_ctrl_data.rc.ch1 = (int16_t)(((int16_t)pData[1] >> 3) | ((int16_t)pData[2] << 5)) & 0x07FF; 
    if(rc_ctrl_data.rc.ch1 > RC_CH_VALUE_MAX || rc_ctrl_data.rc.ch1 < RC_CH_VALUE_MIN) return RC_CH_ERROR;
    rc_ctrl_data.rc.ch1 -= RC_CH_VALUE_OFFSET;

    rc_ctrl_data.rc.ch2 = (int16_t)(((int16_t)pData[2] >> 6) | ((int16_t)pData[3] << 2) | 
                         ((int16_t)pData[4] << 10)) & 0x07FF; 
    if(rc_ctrl_data.rc.ch2 > RC_CH_VALUE_MAX || rc_ctrl_data.rc.ch2 < RC_CH_VALUE_MIN) return RC_CH_ERROR;
    rc_ctrl_data.rc.ch2 -= RC_CH_VALUE_OFFSET;

    rc_ctrl_data.rc.ch3 = (int16_t)(((int16_t)pData[4] >> 1) | ((int16_t)pData[5]<<7)) & 0x07FF;
    if(rc_ctrl_data.rc.ch3 > RC_CH_VALUE_MAX || rc_ctrl_data.rc.ch3 < RC_CH_VALUE_MIN) return RC_CH_ERROR; 
    ...
    rc_ctrl_data.mouse.x = (int16_t)((int16_t)pData[6]) | ((int16_t)pData[7] << 8); 
    rc_ctrl_data.mouse.y = (int16_t)((int16_t)pData[8]) | ((int16_t)pData[9] << 8); 
    rc_ctrl_data.mouse.z = (int16_t)((int16_t)pData[10]) | ((int16_t)pData[11] << 8);     
 
    rc_ctrl_data.mouse.press_left = (uint8_t)pData[12]; 
    rc_ctrl_data.mouse.press_right = (uint8_t)pData[13]; 
  
    rc_ctrl_data.keyboard.keycode = (((uint16_t)pData[14]) | ((uint16_t)pData[15] << 8)); 
...
rc_ctrl_t* get_rc_data_ptr(void){ return &rc_ctrl_data; }
...
/**
  ...
  */
uint32_t is_rc_offline(void){ return rc_rx_lost >= RC_RX_LOST_MAX; }


/**
  * @brief  increment of rc_rx_lost counter, should be call after process of all rc data
  * @return none
  */
void inc_rc_rx_lost(void){
	if(rc_rx_lost < RC_RX_LOST_MAX) rc_rx_lost++;
}
