...
static void* cmdid_buffer_table[CMDID_TABLE_ROW][CMDID_TABLE_COL]={
	{0,NULL,&game_result,NULL,NULL,NULL,0,0,0,0,0},
	{0,NULL,NULL,NULL,NULL,0,0,0,0,0,0},
	{0,&game_robot_status,&power_heat_data,&game_robot_pos,NULL,NULL,&robot_hurt,&shoot_data,NULL,&rfid_status,NULL},
};
...
uint32_t read_referee_data(uint8_t * buf,uint32_t start_offset, uint32_t len){
	...
	for(cur_start =start_offset;cur_start < max_offset; cur_start=next_start){
		...
		if(fh->sof == SOF_BYTE &&
			...
			//提取CMDID高位和低位
			uint8_t cmdid_l = buf[cur_start + CMDID_OFFSET];
			...
			}//else next_start = cur_start + LEN_HEADER;
...
uint16_t send_referee_single(UART_HandleTypeDef* huart,uint16_t cmd_id,uint8_t *p_struct, uint16_t len){
	...
	//memcpy(&tx_buffer[sizeof(xFrameHeader)], (uint8_t*)&cmd_id, sizeof(uint16_t));
